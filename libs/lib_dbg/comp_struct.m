function [fs1, fs2,  er, missing1, missing2] = comp_struct(s1,s2,n1,n2,p,tol)
% Emmanuel Branlard - Mai 2011
%
% Adapted from Michael Arant's script from matlab centra
% 
%
%
...
if nargin == 2; 
    % MANU
    % for first call just use two strings, it easier
    n1=s1;
    n2=s2;
    s1=evalin('base',n1);
    s2=evalin('base',n2);
...
%			different symbolic variables
	...
	if t
        global maxlen
        space=repmat(' ',1,maxlen-length(n1)+6);
% 		disp(sprintf('%s %s OK',n1,space))
	else
        global maxlen
        space=repmat(' ',1,maxlen-length(n1)+6);
		disp(sprintf('%s %s do NOT match',n1,space))
...
%				same size, diferent values or not numbers
				...
				if ~ischar(s1) && ~iscell(s1);
                    ...
                    if(length(s1)>1)
                       ...
                       er=sprintf('%s',['values:  ',num2str(s1),'  ', num2str(s2)]);
                    end
				end
                if(iscell(er))
                    fprintf('%s\n',cell2mat(er));
...
%				tolerance agreement
                global maxlen
                space=repmat(' ',1,maxlen-length(n1)+6);
                disp(sprintf('%s %s tolerance match',n1,space))
			end
		else
%			size difference
            global maxlen
            space=repmat(' ',1,maxlen-length(n1)+6);
			disp(sprintf('%s %s	no -> Different sizes',n1,space))
