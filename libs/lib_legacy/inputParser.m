classdef inputParser < handle
  ...
  methods
    ...
    function addRequired (this, name, val)
      if ~exist('val','var'); val=inputParser.def_val; end;

      if (nargin < 2 || nargin > 3)
        print_usage ();
      elseif (numel (this.Optional) || numfields (this.Parameter) || numfields (this.Switch))
        error ('inputParser.addRequired: can''t have a Required argument after Optional, Parameter, or Switch');
      end
    ...
    function addOptional (this, name, def, val)
      ...
      elseif (numfields (this.Parameter) || numfields (this.Switch))
        error ('inputParser.Optional: can''t have Optional arguments after Parameter or Switch');
      end
    ...
    function addParameter (this, name, def, varargin)
      ...
      if (n_opt == 0 || n_opt == 1)
          ...
          validateattributes (match_priority, {'numeric'}, {'positive', 'integer'}, 'inputParser.addParameter', 'PartialMatchPriority');
      end
  ...
  methods (Access = private)
    function validate_name (this, type, name)
      ...
      elseif (any (strcmpi (this.Parameters, name)))
        ...
        error ('inputParser.add%s: argname %s has already been specified', type, name);
      end
