function Pi = ellipticPiOld(varargin);
% Emmanuel Branlard : July 2013
% adapted from Matlab central (see below)
% interface is identical to mathematica:
% ellipticPi(n,m) or ellipticPi(n,phi,m) 
% but the script below used: ELLIPTIC3(U,M,C)=ellipticPi(phi,m,n)

warning('This script is depreciated')
if(nargin==2)
    u=pi/2;
    n=varargin{1};
    m=varargin{2};
...
elseif(nargin==3)
    ...
    error('input 2 or three arguments')
end
%
...
%   error('M and C must be in the range [0, 1].');
  error('M and N (or C) must be in the range [0, 1].');
end
if any(u > pi/2) | any(u < 0),  
    error('Phase phi (or U) must be in the range [0, pi/2].'); 
end
